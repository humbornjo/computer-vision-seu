...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        assert method=='svd' or method=='eigen'
    ...
    def _eigen_decomp(self, X):
        ...
        # 1/(N-1) coefficient to correctly run the kernels in the ipynb file.
        # not necessary.
        e_vals,e_vecs=np.linalg.eig(X.T@X/(N-1))
        idx=np.argsort(-e_vals)
        # take the real part to avoid error complex128 cannot convert to float
    ...
    def transform(self, X, n_components):
        ...
        X_proj=(X-self.mean)@self.W_pca[:,:n_components]
        pass
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        X=X_proj@np.linalg.inv(self.W_pca)[:n_components,:]+self.mean
        pass
        # END YOUR CODE
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        S_w=self._within_class_scatter(X,y)
        S_b=self._between_class_scatter(X,y)
        # solve (S_w)^-1*S_b
        e_vals,e_vecs= scipy.linalg.eig(np.linalg.inv(S_w)@S_b)
        idx=np.argsort(-e_vals)
        self.W_lda=e_vecs[:,idx]
        pass
        # END YOUR CODE
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            # get all the samples with index i
            idx=(y==i)
            X_i=X[idx]
            X_centered=X_i-np.mean(X_i,axis=0)
            # sum covariance matrices up
            scatter_within+=X_centered.T@X_centered
            pass
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            # mu_i and mu with shape (D,)
            # reshape them to (1,D) using np.newaxis
            mu_centered=mu-mu_i[np.newaxis,:]
            # N_i = len(idx)
            scatter_between+=len(idx)*mu_centered.T@mu_centered
            pass
            # END YOUR CODE
