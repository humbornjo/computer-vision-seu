...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        y_j, x_j = y + w + 1, x + w + 1
        # shape the matrix into the one in the ipynb
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        y1_i, x1_i = max(y1 - w, 0), max(x1 - w, 0)
        y1_j, x1_j = y1 + w + 1, x1 + w + 1
        A = np.hstack((Ix[y1_i:y1_j, x1_i:x1_j].reshape(-1, 1), Iy[y1_i:y1_j, x1_i:x1_j].reshape(-1, 1)))
        pass
        ### END YOUR CODE
        ...
        for k in range(num_iters):
            ...
            #print(img2.shape,x2_i,x2_j,img2[y2_i:y2_j, x2_i:x2_j].shape,img1[y1_i:y1_j, x1_i:x1_j].shape)
            bk = (img2[y2_i:y2_j, x2_i:x2_j]-img1[y1_i:y1_j, x1_i:x1_j]).reshape(-1, 1)
            # least square
            vk=(np.linalg.inv(A.T@A)@A.T@-bk).flatten()
            pass
            ### END YOUR CODE
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        ...
        # not necessary to predict the last turn
        if L > 0:
            g = scale * (g + d)
        pass
        ### END YOUR CODE
...
def IoU(bbox1, bbox2):
    ...
    ### YOUR CODE HERE
    y = min(y1 + w1, y2 + w2) - max(y1, y2)
    x = min(x1 + h1, x2 + h2) - max(x1, x2)
